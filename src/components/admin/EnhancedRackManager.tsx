...
import { getRacks, createRack, updateRack, deleteRack, getAllRacksForStore } from '../../services/firestoreService';
import { Package, Plus, Edit, Trash2, Tag, Save, X, Search } from 'lucide-react';

interface EnhancedRackManagerProps {
  store: Store;
  floors: Floor[];
}

export const EnhancedRackManager: React.FC<EnhancedRackManagerProps> = ({ store, floors }) => {
  ...
  const loadAllRacks = async () => {
    try {
      const storeRacks = await getAllRacksForStore(store.id);
      setAllRacks(storeRacks);
    } catch (error) {
      console.error('Error loading all racks:', error);
    }
  };
    ...
    <div className="p-6">
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search racks or products..."
      ...
      {allRacks.length > 0 && (
        <div className="mt-8 p-4 bg-blue-50 rounded-lg">
          <h4 className="font-medium text-blue-900 mb-2">Store Summary</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <span className="text-blue-600 font-medium">Total Racks:</span>
              <div className="text-blue-800">{allRacks.length}</div>
            </div>
            <div>
              <span className="text-blue-600 font-medium">Total Products:</span>
              <div className="text-blue-800">
                {allRacks.reduce((sum, rack) => sum + rack.products.length, 0)}
              </div>
            </div>
            <div>
              <span className="text-blue-600 font-medium">Floors:</span>
              <div className="text-blue-800">{floors.length}</div>
            </div>
            <div>
              <span className="text-blue-600 font-medium">Avg Products/Rack:</span>
              <div className="text-blue-800">
                {allRacks.length > 0 
                  ? Math.round(allRacks.reduce((sum, rack) => sum + rack.products.length, 0) / allRacks.length)
                  : 0
                }
              </div>
            </div>
